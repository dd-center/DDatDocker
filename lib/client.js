...
class Client extends EventEmitter {
  constructor(config, { fetchImpl = fetch, WebSocketImpl = WebSocket, random = Math.random, log = () => {} } = {}) {
    ...
    this.stats = { connections: 0, disconnects: 0, received: 0, submitted: 0, valid: 0, failed: 0, cancelled: 0, overloaded: 0, malformed: 0, lastTaskAt: null, lastValidAt: null, lastFailure: null }
  }
  ...
  async processTask(s, key, url) {
    ...
    try {
      ...
      if (!valid) {
        error = `HTTP ${result.status}, API code ${code ?? 'non-JSON'}`
