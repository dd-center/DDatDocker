...
本机真实调度服务冒烟测试（60 秒）：连接持续正常，服务端 `homes` 可见诊断昵称及运行环境；收到的任务返回 HTTP 200、B 站 API code `-352`，服务端记录 `resolves=0`、`rejects=1`。维护版识别为失败并进入冷却，没有反复重连。这次测试验证了连接和上报协议，**没有证明该网络下采集成功**。B 站访问条件需在目标部署环境继续验证。

后续补查确认 `/x/space/wbi/acc/info` 返回 `-352: 风控校验失败`；`/guard/topList` 曾返回 `code: 0` 并被本地提交，但该短检查没有等待服务端成功计数，不能当作确认接收。另一次 120 秒完整测试的服务端计数仍为 `resolves=0`、`rejects=4`，其中 `/xlive/web-room/v1/index/getInfoByRoom` 也返回 `-352`。因此真实接口兼容/风控问题仍是未完成的生产验收项，而不是已解决的客户端故障。

## 实测资源与可重复结果

[AMD64/ARM64 Compose 验证记录](https://github.com/dd-center/DDatDocker/actions/runs/35438861984) 包含零配置启动、重建后身份保留、断线恢复、进程冻结后的重启和正常停止。27 项自动测试全部通过。

第一轮原生容器测试在 `LIMIT=0`、每 100 ms 拉取一个本地小 JSON 任务的条件下得到：

| 架构 | Docker 统计内存 | 镜像解包大小（首轮，补充 Node 许可证前） |
| --- | ---: | ---: |
| AMD64 | 21.99 MiB | 143,398,529 bytes（136.8 MiB） |
| ARM64 | 21.35 MiB | 140,035,858 bytes（133.5 MiB） |

这不是开启 5 个繁忙直播房间后的内存，也不包含 Docker Desktop 虚拟机自身的开销。生产依赖目录约 200 KB，业务模块约 48 KB；镜像主要体积来自 Node 本身，不能把它称为 Go 静态二进制那样小的镜像。最终打包额外保留约 154 KiB 的 Node 许可证。

本地 120 秒加速故障注入：171 次强制断线、172 次连接、6,257 个有效模拟任务、858 个预期失败、199 个因会话结束取消的任务、665 个被容错处理的损坏消息；并发峰值 3，符合该测试配置。该数字用于说明恢复路径经受过反复执行，不构成长期无故障保证。
